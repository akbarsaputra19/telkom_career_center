...
class UpdatePhotoCubit extends Cubit<UpdatePhotoState> {
  ...
  Future<void> fetchProfileUpdatePhoto(File image) async {
    emit(UpdatePhotoInitial());
    final token = await Commons().getUid();

    final response = await repository.fetchProfileUpdatePhoto(AuthenticationHeadersRequestUpload(token), image);

    if (response is ResultSuccess) {
      emit(UpdatePhotoIsSucces(message: 'Update Photo Success')
      );
    } else {
